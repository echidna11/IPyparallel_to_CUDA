...
'''
simple GA based refinement of selected aligned subtomograms for getting higher quality average

~/ln/tomominer/tomominer/average/genetic_algorithm/aligned_refine.py
...
def average(dj, mask_count_threshold):
    ...
    # the following part is simplified from tomominer.pursuit.multi.util.vol_avg__local()
    ...
    for d in dj:
        ...
        if not N.all(N.isfinite(v)):        raise Exception('error loading', d['subtomogram'])

        vm = IF.read_mrc_vol(d['mask'])

        v_r = GR.rotate_pad_mean(v, angle=d['angle'], loc_r=d['loc'])       ;           assert N.all(N.isfinite(v_r))
        vm_r = GR.rotate_mask(vm, angle=d['angle'])                    ;           assert N.all(N.isfinite(vm_r))
...
def main():
    with open('aligned_refine__op.json') as f:      op = json.load(f)
    ...
    if not os.path.isdir(avg_dir):      os.makedirs(avg_dir)
    IF.put_mrc(avg_re['v'], os.path.join(avg_dir, 'vol_avg.mrc'))
    IF.put_mrc(avg_re['m'], os.path.join(avg_dir, 'mask_avg.mrc'))
    
    with open(os.path.join(out_dir, 'data_selected.json'), 'w') as f:        json.dump(pmpg['dj'], f, indent=2)
