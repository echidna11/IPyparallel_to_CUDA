...
~/ln/tomominer/tomominer/json_/list_/select_equal.py
...
def main():
    ...
    with open('json_list_select_equal__op.json') as f:   op = json.load(f)
    ...
    for ri, rec in enumerate(rec_json):
        if get_field_val(rec, op['fields']) != op['value']:     continue
