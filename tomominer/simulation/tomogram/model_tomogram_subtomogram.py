...
def model_processing(op):
    ...
    #To skip loading of tomorgams and density files, directly check whether info file is available, because it is generated at the end of subtomogram extraction
    if op['regenerate'] == "False":
        if 'peak' in op:
    ...
    # if 'peak' not in config file then no particle picking, just extract ground truth
        ...
        if 'size_ratio' in op['subtomogram']:
            assert      'size' not in op['subtomogram']
            # take diameter of biggest particle and set size depending on that
            ...
            print '# use max diameter of ground truth commplexes to set subtomogram size', op['subtomogram']['size']
        ...
        subtomogram_paths = PSE.subtomogram_extraction(v=vt, x=peak_re['x'], r=(N.array(peak_re['r'])*op['subtomogram']['ground truth mask ratio'] if ('ground truth mask ratio' in op['subtomogram']) else None), siz=N.array(op['subtomogram']['size']), out_dir=out_dir)
