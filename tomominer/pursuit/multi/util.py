...
def cluster_average_select_fsc(self, cluster_info, cluster_info_stat, op=None, debug=False):
    ...
    for i in cluster_info:
        for c in cluster_info[i]:
            ...
            if (not op['keep_non_specific_clusters']) and ('is_specific' in cluster_info_stat[i][c]) and (cluster_info_stat[i][c]['is_specific'] is not None):
                continue      # ignore non-specific clusters, if keep_non_specific_clusters==False

            ci.append( cluster_info[i][c] )        

    # use float to make sure that the value used is a single scalar, not a numpy array, in order to avoid confusion in sorting
    # soreted function sorts from smaller to bigger number, minus sign makes it bigger to smaller
    ...
    for ci_t in ci:
        if 'template_key' not in ci_t:
            continue # every cluster must have a template key, either itself or some other cluster

        subtomograms_t = set(_['subtomogram'] for _ in ci_t['data_json'])

        # fraction of subtomograms of cluster already present in covered_set (set of subtomogram of clusters present in selected list i.e. ci_cover)
    ...
    for i in cluster_info:
        for c in cluster_info[i]:
            ...
            if tk_subtomogram not in tk_selected:
                continue    # to reduce storage, we only keep information of templates selected in current pass
...
'''
############################################################################################################
# I added out_path as arguement for function template_segmentation__singla_vol, because it used out_path to print warnings and it doesn't knowe what out_path is
############################################################################################################
