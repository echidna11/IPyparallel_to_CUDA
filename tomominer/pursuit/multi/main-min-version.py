...
def stop_test_stat(op, fsc_stat, pass_i, cluster_modes):
    ...
    if op['options']['stopping_test']['criterion'] == 0:
        #print '# stopping test according to selected_templates_max_pass_i (of only non-reduenant clusters)'
        ...
        for _, _fsc_stat in fsc_stat.iteritems():
            for  __, __fsc_stat in _fsc_stat.iteritems():
                ...
                if __fsc_stat['pass_i'] > selected_templates_max_pass_i:        selected_templates_max_pass_i = __fsc_stat['pass_i']
...
def pursuit(self, op, data_json):
    ...
    if os.path.exists(data_json_file):
        ...
        for rec in data_json:
            if ('loc' not in rec) or op['random_initial_alignment']: rec['loc'] = [0.0, 0.0, 0.0]
            if ('angle' not in rec) or op['random_initial_alignment']: rec['angle'] = [_ for _ in N.random.random(3) * (N.pi * 2)]
    ...
    cluster_info_stat = defaultdict(dict)        # in order to save storage, we store those information that may subject to change here, for example 'is_specific'
    ...
    for pass_i in range(op['options']['pass_num']):
        ...
        with open(op_file_t, 'w') as f:        json.dump(op, f, indent=2)          # keep a copy of the configuration
        #file_stat['passes'][pass_i]['op_file'] = op_file_t
        ...
        if os.path.exists(dimension_reduction_dump_file):
            ...
            if (op['dim_reduction']['train_with_selected_clusters_only']) and (cas_re is not None):
                #print '# use the subtomograms of the selected clusters from last iteration (with latest alignment) for dimension reduction', ('only specific clusters' if op['dim_reduction']['restrict_to_specific_clusters'] else '')
                
                data_json_pca_train__set = set()
                data_json_pca_train = []
                for c in cas_re['selected_templates']:
                    t = cas_re['tk_info'][cas_re['selected_templates'][c]['subtomogram']]
                    if op['dim_reduction']['restrict_to_specific_clusters'] and (cluster_info_stat[t['pass_i']][t['cluster']]['is_specific'] is not None):  continue        # if this cluster is selected in last iteration, but identified as non-specific, ignore it
                    for d in t['data_json']:
                        if d['subtomogram'] in data_json_pca_train__set:    continue
                        data_json_pca_train__set.add(d['subtomogram'])
                        data_json_pca_train.append(data_json_dict[d['subtomogram']])        # add the corresponding subtomogram of specific clusters selected from the last iteration, with latest alignment information. The use of latest alignment info is because we want to use dimension reduction and clustering to obtain new and better clusters
        ...
        # update collected cluster statistics
...
todo: use UUID to index subtomograms, and use redis to retrive subtomograms
