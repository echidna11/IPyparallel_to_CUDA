...
class Model(object):
    ...
    def solve_coeffs(self):
        ...
        n_chunk = int(float(self.nobs) / self.pool._processes) + 1

        i_s = list(range(self.nobs))

        pre = []
        while i_s:
            i_st = i_s[:n_chunk]
            pre.append( self.pool.apply_async(func=_solve_coeffs__single_job, kwds={'eigvec':self.eigvec, 'weights':self.weights[i_st,:], 'data':self.data[i_st,:], 'i_s':i_st}) )
            i_s = i_s[n_chunk:]

        for r in pre:
            r = r.get(999999)
            self.coeff[r['i_s']] = r['coeff']

        if False:
            # for debugging, use the following original version for verification
            coeff_t = N.zeros(self.coeff.shape)

            for i in range(self.nobs):
                #- Only do weighted solution if really necessary
                if N.any(self.weights[i] != self.weights[i,0]):
                    coeff_t[i] = _solve(self.eigvec.T, self.data[i], self.weights[i])
                else:
                    coeff_t[i] = N.dot(self.eigvec, self.data[i])
            print N.abs(coeff_t - self.coeff).max()
    ...
    def solve_eigenvectors(self, smooth=None):
        ...
        for k in range(self.nvec):
            ...
            js = list(range(self.nvar))

            n_chunk = int(float(self.nvar) / self.pool._processes) + 1

            pre = []
            while js:
                jst = js[:n_chunk]
                pre.append( self.pool.apply_async(func=_solve_eigenvectors__single_job, kwds={'c':c, 'weights':self.weights[:,jst], 'data':data[:,jst], 'js':jst}) )
                js = js[n_chunk:]

            for r in pre:
                r = r.get(999999)
                self.eigvec[k, r['js']] = r['eigvec']

            if False:
                # for debugging, use the following original version for verification
                eigvec_t = N.zeros(self.nvar)
                for j in range(self.nvar):
                    ...
                    cw = c*w
                    eigvec_t[j] = x.dot(cw) / c.dot(cw)
                print N.abs(self.eigvec[k,:].flatten() - eigvec_t).max()
    ...
    def solve_model(self):
        ...
        while i_s:
            i_st = i_s[:n_chunk]
            pre.append( self.pool.apply_async(func=_solve_model__single_job, kwds={'eigvec_t':eigvec_t, 'coeff':self.coeff[i_st,:], 'i_s':i_st}) )
            i_s = i_s[n_chunk:]

        for r in pre:
            r = r.get(999999)
            self.model[r['i_s'], :] = r['model']

        if False:
            # for debugging, use the following original version for verification
            model_t = N.zeros(self.model.shape)
            for i in range(self.nobs):
                model_t[i] = eigvec_t.dot(self.coeff[i])

            print N.abs(model_t - self.model).max()
...
def _solve_eigenvectors__single_job(c, weights, data, js):
    nvar = weights.shape[1]     ;       assert  data.shape[1] == nvar
    eigvec = N.zeros(nvar)
    for j in range(nvar):
        w = weights[:, j]
        x = data[:, j]
        cw = c*w
        eigvec[j] = x.dot(cw) / c.dot(cw)

    return {'eigvec':eigvec, 'js':js}

def _solve_coeffs__single_job(eigvec, weights, data, i_s):
    ...
    for i in range(len(data)):
        #- Only do weighted solution if really necessary
        if N.any(weights[i] != weights[i,0]):
            coeff[i] = _solve(eigvec_t, data[i], weights[i])
        else:
            coeff[i] = N.dot(eigvec, data[i])

    return {'coeff':N.array(coeff), 'i_s':i_s}



def _solve_model__single_job(eigvec_t, coeff, i_s):

    model = [None] * len(coeff)
    for i in range(len(coeff)):
        model[i] = eigvec_t.dot(coeff[i])

    return {'model':N.array(model), 'i_s':i_s}
