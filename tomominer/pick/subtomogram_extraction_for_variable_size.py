...
def subtomogram_extraction(v, x, out_dir, radius, op, ids=None, max_file_num_per_dir=100):
    ...
    diameter_max = max(radius) * 2
    siz = N.zeros((len(x),3), dtype=N.int16)
    if 'size_ratio' in op['subtomogram']:
        assert 'size' not in op['subtomogram']
        for i in range(len(x)): 
            siz[i,:] = N.array([  int(N.ceil(radius[i] * op['subtomogram']['size_ratio']))  ] * 3)
        max_size = diameter_max * op['subtomogram']['size_ratio']

    elif 'size' in op['subtomogram']:
        assert      'size_ratio' not in op['subtomogram']
        for i in range(len(x)):
            siz[i,:] = N.array([ op['subtomogram']['size'] ]*3)
    ...
    for i in range(len(x)):
        ...
        vc = CV.cut_from_whole_map(whole_map=v, c=x[i,:].flatten(), siz=siz[i,:].flatten())
        ...
        # Code to place zeros at sides of extracted subtomogram to make all subtomograms of same size
        # Alternativly noise can also be used. But as it is processing step its better to put zeros
        # One can also try to run MPP directly without appending zeros
        if 'size_ratio' in op['subtomogram']:
            new_vc = N.zeros((max_size, max_size, max_size), dtype=N.int16)
            #rng is for range
            rng = (max_size-vc.shape[0])/2
            for i in range(vc.shape[0]):
                for j in range(vc.shape[0]):
                    for k in range(vc.shape[0]):
                        new_vc[i+rng, j+rng, k+rng] = vc[i,j,k]
