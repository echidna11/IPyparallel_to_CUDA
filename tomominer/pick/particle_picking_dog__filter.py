...
def do_filter(pp, peak_dist_min, op):
    ...
    for peak_i in range(len(pp)):
        ...
        if ('top_num' in op) and (peak_i > (op['top_num']*2)):
    ...
    for i,pp_t in enumerate(pp):
        ...
        if ('top_num' in op) and (len(pp_f) >= op['top_num']):     break
...
def main():
    import json
    with open('particle_picking_dog__filter__config.json') as f:    op = json.load(f)
    ...
    with open(op['out_file'], 'w') as f:       json.dump(pp_t, f, indent=2)
