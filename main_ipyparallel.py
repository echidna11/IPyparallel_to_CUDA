...
#import tomominer.pursuit.multi.ipyparallelUtil as CU
import ipyparallelUtil as CU
import shutil, random, uuid, gc
import pickle
import time, os, copy, json, sys
import numpy as N
    ...
    for pass_i in range(op['options']['pass_num']):
        ...
        if pass_i == 0:
            ...
            if os.path.exists(cluster_dump_file):
                ...
                if not os.path.isfile(cluster_kmeans_dump_file):    
                    ...
                    for pass_0 in range(op['options']['num_of_first_iterations']):
                        ...
                        if os.path.exists(dimension_reduction_dump_file):
                            ...
                            if op['dim_reduction']['with_missing_wedge']:
                                ...
                                cfp_re = CU.covariance_filtered_pca_ipyparallel(self=self, data_json_model=data_json_pca_train, data_json_embed=data_json_copy, normalize=op['dim_reduction']['normalize'], segmentation_tg_op=(segmentation_tg_op if op['dim_reduction']['use_segmentation_mask'] else None), pca_op=op['dim_reduction']['pca'], max_feature_num=op['dim_reduction']['max_feature_num'])
                    ...
                    cluster_ssnr_fsc = CU.cluster_ssnr_fsc(self=self, clusters={_:kmeans_clusters[_]['data_json'] for _ in kmeans_clusters}, op=cluster_ssnr_fsc__op)
            ...
            # creating dimension reduction file
                ...
                if op['dim_reduction']['with_missing_wedge']:
                    ...
                    cfp_re = CU.covariance_filtered_pca_ipyparallel(self=self, data_json_model=data_json_pca_train, data_json_embed=data_json, normalize=op['dim_reduction']['normalize'], segmentation_tg_op=(segmentation_tg_op if op['dim_reduction']['use_segmentation_mask'] else None), pca_op=op['dim_reduction']['pca'], max_feature_num=op['dim_reduction']['max_feature_num'])
        ...
        if os.path.exists(cluster_average_select_file):
            ...
            if op['common_frame']['mode'] == 1:
                caacfmp = CU.cluster_average_align_common_frame__multi_pair(tk=cas_re['selected_templates'], align_op=op['align'], loc_r_max=size.min()*float(op['common_frame']['loc_r_max_proportion']), pass_dir=pass_dir)
            ...
            if segmentation_op is not None:
                ...
                CU.template_segmentation(tk=cas_re['selected_templates_common_frame'], op=template_segmentation_op)
        ...
        if os.path.isfile(align_template_file):
            ...
            at_ress = CU.align_to_templates__batch_ipyparallel(op=op, data_json=data_json, segmentation_tg_op=segmentation_tg_op, tmp_dir=align_template__tmp_dir, tem_keys=cas_re['selected_templates_common_frame'])
