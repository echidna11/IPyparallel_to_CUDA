...
#import tomominer.statistics.ssnr_ipyparallel as SS
...
import ipyparallel, zmq, gc
import numpy as N
import numpy.fft as NF
from collections import defaultdict
import os, sys, copy, time, traceback
import warnings, uuid, shutil, pickle
...
def align_keys_without_cache(v1k, v2k, op, v2_out_key=None):

    import tomominer.io.file as IV
    import ipyparallelUtil
    import tomominer.geometry.rotate as GR

    if op['with_missing_wedge']:
        v1  = IV.get_mrc(v1k['subtomogram'])
        if 'mask' in v1k:   
            m1 = IV.get_mrc(v1k['mask'])
        else:   
            m1 = None

        v2  = IV.get_mrc(v2k['subtomogram'])
        if 'mask' in v2k:   
            m2 = IV.get_mrc(v2k['mask'])
        else: 
            m2 = None

        re = ipyparallelUtil.align_vols_with_wedge(v1, m1, v2, m2, op=op)

    else:
        v1  = IV.get_mrc(v1k['subtomogram'])
        v2  = IV.get_mrc(v2k['subtomogram'])
        re = ipyparallelUtil.align_vols_no_wedge(v1=v1, v2=v2, op=op)
...
def rotate_key_without_cache(vk, vk_out, angle, loc):
...
def impute_vol_keys_without_cache(vk, ang, loc, normalize, tk=None, align_to_template=True):

    v  = IV.get_mrc(vk['subtomogram'])
    if not N.all(N.isfinite(v)):        raise Exception('error loading', vk['subtomogram'])

    vm = IV.get_mrc(vk['mask'])
...
def neighbor_covariance_avg__ipyparallel(self, data_json, segmentation_tg_op, normalize):
    ...
    # Split the arrays into chunks of 5
    data_chunks = [data_json_copy[i:i+n_chunk] for i in range(0, len(data_json_copy), n_chunk)]
    fnames = [os.path.join(self.cache.tmp_dir, "tmp-"+str(uuid.uuid1())+".pickle") for i in range(len(data_chunks))]

    # Use the map function to apply the add_arrays function to each chunk
    result_filenames = view.map_sync(neighbor_covariance__collect_info_ipyparallel, data_chunks, [segmentation_tg_op] * len(data_chunks), [normalize] * len(data_chunks), fnames)
    #result_files = view.gather(results)
    print("Tasks submitted:", len(data_chunks))
    print("Files saved:", len(result_filenames))
    ...
    for fname in result_filenames:
        print "reading", fname
        with open(fname, "rb") as f:     re = pickle.load(f)
...
def neighbor_covariance_avg__ipyparallel_1(self, data_json, segmentation_tg_op, normalize):

    start_time = time.time()

    data_json_copy = [_ for _ in data_json]
    # inds = range(len(data_json_copy))
    # import ipyparallel, zmq
    rc = ipyparallel.Client(context=zmq.Context())
    ...
    while data_json_copy:
        ...
        async_result = view.apply_async(f=neighbor_covariance__collect_info_ipyparallel, data_json=data_json_copy_part, segmentation_tg_op=segmentation_tg_op, normalize=normalize, fname=fname)
        async_results.append(async_result)
        data_json_copy = data_json_copy[n_chunk:]
        # inds = inds[n_chunk:]

    print(len(result_files), " tasks created")
    rc.wait_interactive(async_results)

    sum_global = None
    neighbor_prod_sum = None
    shift = None
    # while len(result_files)!=N.sum([os.path.isfile(_) for _ in result_files]):
    #     print "waiting for all tasks to save files..."
    #     sys.stdout.write("\033[F")
    #     continue
    # for ar in async_results:
    #     assert ar.get() in result_files
    # del async_results
    # gc.collect()
...
def neighbor_covariance_avg__ipyparallel_2(self, data_json, segmentation_tg_op, normalize):
    ...
    # Use the map function to apply the add_arrays function to each chunk
    async_results = [view.apply_async(neighbor_covariance__collect_info_ipyparallel, data_chunk, segmentation_tg_op, normalize, fname) for data_chunk, segmentation_tg_op, normalize, fname in zip(data_chunks, [segmentation_tg_op] * len(data_chunks), [normalize] * len(data_chunks), fnames)]
    # result_filenames = view.map(neighbor_covariance__collect_info_ipyparallel, data_chunks, [segmentation_tg_op] * len(data_chunks), [normalize] * len(data_chunks), fnames, block=True)
    # result_files = view.gather(results)
    print("Tasks submitted:", len(data_chunks))
        ...
        for i, ar in enumerate(async_results):
            if ar.ready():
                ...
                if neighbor_prod_sum is None:
                    ...
                    neighbor_prod_sum += re['neighbor_prod_sum']
                assert  N.all(N.isfinite(neighbor_prod_sum))
                print str(n_results) + " out of " + str(len(async_results)) + " tasks completed"
                del re
                #gc.collect()                
                os.remove(fname)
                n_results += 1
    ...
    del async_results, fnames, data_chunks
    gc.collect()
    ...
    print 'Calculated neighbor covariance for', len(data_json), 'subtomograms', ' : %2.6f sec' % (time.time() - start_time)
    del avg_global, neighbor_prod_avg, cov, sum_global, neighbor_prod_sum, shift
    gc.collect()

    return cov_avg

# collecting information for calculating the neighbor covariance, calculated at worker side
def neighbor_covariance__collect_info_ipyparallel(data_json, segmentation_tg_op, normalize, fname):

    # from tomominer.pursuit.multi.ipyparallelUtil import ipyparallelUtil
    import ipyparallelUtil as CU
    import numpy as N, pickle, gc
    import tomominer.dimension_reduction.util as DU
    import tomominer.io.file as IV
    ...
    for rec in data_json:
        ...
        vri = CU.impute_vol_keys_without_cache(vk=rec, ang=rec['angle'], loc=rec['loc'], tk=rec['template'], align_to_template=True, normalize=normalize)['vi']
        ...
        if (segmentation_tg_op is not None) and (rec['template'] is not None) and ('segmentation' in rec['template']):
            ...
            vri_s = CU.template_guided_segmentation(v=vri, m=(phi>0.5), op=segmentation_tg_op)          # why phi>0.5 instead of 0? Because we do not want to include boundary region?
    ...
    with open(fname, "wb") as f:
        pickle.dump(re, f)
    del re, sum_local, neighbor_prod_sum, nei_prod
    gc.collect()
    return fname

# modified from tomominer.dimension_reduction.util.pca__stack_dif__parallel()
def data_matrix_collect__ipyparallel(self, data_json, segmentation_tg_op, normalize, voxel_mask_inds=None):
    start_time = time.time()
    ...
    # Split the arrays into chunks of 5
    data_chunks = [data_json_copy[i:i+n_chunk] for i in range(0, len(data_json_copy), n_chunk)]
    inds_chunks = [inds[i:i+n_chunk] for i in range(0, len(inds), n_chunk)]
    fnames = [os.path.join(self.cache.tmp_dir, "tmp-"+str(uuid.uuid1())+".pickle") for i in range(len(data_chunks))]
    
    # Use the map function to apply the add_arrays function to each chunk
    async_results = [view.apply_async(data_matrix_collect__local_ipyparallel, data_chunk, inds_chunk, segmentation_tg_op, normalize, fname, voxel_mask_ind) for data_chunk, inds_chunk, segmentation_tg_op, normalize, fname, voxel_mask_ind in zip(data_chunks, inds_chunks, [segmentation_tg_op] * len(data_chunks), [normalize] * len(data_chunks), fnames, [voxel_mask_inds] * len(data_chunks))]
    # result_filenames = view.map(data_matrix_collect__local_ipyparallel, data_chunks, inds_chunks, [segmentation_tg_op] * len(data_chunks), [normalize] * len(data_chunks), fnames, [voxel_mask_inds] * len(data_chunks), block=True)
    # result_files = view.gather(results)
    print("Tasks submitted:", len(data_chunks))
    # print("Files saved:", len(result_filenames))    
    ...
    del async_results, fnames, data_chunks, inds_chunks
    gc.collect()
...
def data_matrix_collect__local_ipyparallel(data_json, inds, segmentation_tg_op, normalize, fname, voxel_mask_inds=None):
    ...
    for i, rec in enumerate(data_json):
        ...
        vi = ipyparallelUtil.impute_vol_keys_without_cache(vk=rec, ang=rec['angle'], loc=rec['loc'], tk=rec['template'], align_to_template=True, normalize=normalize)['vi']

        if (segmentation_tg_op is not None) and (rec['template'] is not None) and ('segmentation' in rec['template']):
            #print 'apply template segmentation guide to the interpolated subtomgoram', rec['template']['segmentation']
            phi = IV.read_mrc(rec['template']['segmentation'])['value']

            vi_s = ipyparallelUtil.template_guided_segmentation(v=vi, m=(phi>0.5), op=segmentation_tg_op)      # why phi>0.5 instead of 0? Because we do not want to include boundary region?
            if vi_s is not None:
                vi = vi_s;                  del vi_s
...
def covariance_filtered_pca_ipyparallel(self, data_json_model=None, data_json_embed=None, normalize=None, segmentation_tg_op=None, max_feature_num=None, pca_op=None):
    ...
    if (max_feature_num==None) or (max_feature_num < 0):
        ...
        cov_avg = neighbor_covariance_avg__ipyparallel_2(self=self, data_json=data_json_model, segmentation_tg_op=segmentation_tg_op, normalize=normalize)
    ...
    mat = data_matrix_collect__ipyparallel(self=self, data_json=data_json_model, segmentation_tg_op=segmentation_tg_op, normalize=normalize, voxel_mask_inds=voxel_mask_inds)
    ...
    if (data_json_embed is None) or ():
        ...
        mat_embed = data_matrix_collect__ipyparallel(self=self, data_json=data_json_embed, segmentation_tg_op=segmentation_tg_op, normalize=normalize, voxel_mask_inds=voxel_mask_inds)
...
def kmeans_clustering(x, k):
    ...
    if False:
        ...
        km = KMeans(n_clusters=k, n_init=10, n_jobs=int(multiprocessing.cpu_count()/3))       # by default, such kmeans method uses kmeans++ initialization, and try 10 times to reduce the chance of falling to local minima. In such case, hopfully we have a high chance to get clusters inside dense regions of data points, becase this makes the objective function have a higher value
...
def cluster_ssnr_fsc(self, clusters, op=None):
    start_time = time.time()
    import ssnr_ipyparallel as SS
    sps = SS.ssnr_parallel(self=self, clusters=clusters, op=op)

    #print 'cluster_ssnr_fsc took time %.1f'%(time.time() - start_time)

    return sps

def vol_avg__local_ipyparallel(data_json, fname, op=None):
    ...
    for rec in data_json:
        if op['with_missing_wedge'] or op['use_fft']:
            ...
            in_re = ipyparallelUtil.impute_vol_keys_without_cache(vk=rec, ang=rec['angle'], loc=rec['loc'], tk=None, align_to_template=True, normalize=False)
        ...
        # else use templates (cluster average from last iteration) stored as part of data_json, to impute the subtomograms and interpolate and use those subtomograms for averaging
            ...
            if 'template' not in rec:       rec['template'] = None
            in_re = ipyparallelUtil.impute_vol_keys_without_cache(vk=rec, ang=rec['angle'], loc=rec['loc'], tk=rec['template'], align_to_template=True, normalize=True)
            vt = in_re['vi']
...
def cluster_averaging_vols_ipyparallel(self, clusters, op={}):
    ...
    for c in clusters:
        while clusters[c]:
            part = clusters[c][:n_chunk]
            fname = os.path.join(self.cache.tmp_dir, "tmp-"+str(uuid.uuid1())+".pickle")
            fnames.append(fname)
            op_t = copy.deepcopy(op)
            op_t['cluster'] = c
            async_result = view.apply_async(f=vol_avg__local_ipyparallel, data_json=part, op=op_t, fname=fname)
            async_results.append(async_result)
            clusters[c] = clusters[c][n_chunk:]
...
def cluster_averaging(self, clusters, op={}):
    ...
    cav =  cluster_averaging_vols_ipyparallel(self, clusters=clusters, op=op)
...
def cluster_average_align_common_frame__pairwise_alignment_ipyparallel(template_keys, align_op):
    ...
    for c0 in template_keys:
        for c1 in template_keys:
            if c1 <= c0: continue
            async_result = view.apply_async(f=align_keys_without_cache, v1k=template_keys[c0], v2k=template_keys[c1], op=align_op)
            async_results.append(async_result)

    print(len(async_results), " tasks created")
...
def cluster_average_align_common_frame__multi_pair(tk, align_op, loc_r_max, pass_dir):
    ...
    # first perform pairwise alignment
    pa = cluster_average_align_common_frame__pairwise_alignment_ipyparallel(template_keys=tk, align_op=align_op)
    ...
    for c in tk:
        ...
        if c in align_to_clus:
            # align c with the selected template
            rotate_key_without_cache(vk=tk[c], vk_out=tka[c], angle=pa[align_to_clus[c]][c]['angle'], loc=pa[align_to_clus[c]][c]['loc'])
...
def template_segmentation__single(c, tk, op):
    ...
    phi = template_segmentation__single_vol(v=v, op=op, out_path=out_path)
...
def template_segmentation__single_vol(v, op, out_path):
...
def template_segmentation__single_ipyparallel(c, tk, op):
    ...
    phi = ipyparallelUtil.template_segmentation__single_vol(v=v, op=op, out_path=out_path)
...
def template_segmentation_ipyparallel(tk, op):
    ...
    for c in tk:
        async_result = view.apply_async(f=template_segmentation__single_ipyparallel, c=c, tk=copy.deepcopy(tk[c]), op=copy.deepcopy(op))
        async_results.append(async_result)
...
def template_segmentation(tk, op, multiprocessing=False):
    for c in tk:
        print "segmenting:", c
...
# def align_to_templates__batch_ipyparallel(op, data_json, segmentation_tg_op, tmp_dir, tem_keys):

#     print 'align against templates', 'segmentation_tg_op', (segmentation_tg_op if op['template']['match']['use_segmentation_mask'] else None)       ;       sys.stdout.flush()
...
#     completed_subtomogram_set = set([_['vol_key']['subtomogram'] for _ in at_ress])
...
#     for rec in data_json:
#         # print rec["subtomogram"]
#         if rec['subtomogram'] in completed_subtomogram_set:     continue            # ignore those completed alignments
#         fname = os.path.join(tmp_dir, "tmp-"+str(uuid.uuid1())+".pickle")
#         fnames.append(fname)
#         # at_ress_t = align_to_templates_ipyparallel(rec=rec, segmentation_tg_op=(segmentation_tg_op if op['template']['match']['use_segmentation_mask'] else None), tem_keys=tem_keys, align_op=op['align'], multiprocessing=False, fname=fname)
#         async_result = view.apply_async(f=align_to_templates_ipyparallel, rec=rec, segmentation_tg_op=(segmentation_tg_op if op['template']['match']['use_segmentation_mask'] else None), tem_keys=tem_keys, align_op=op['align'], multiprocessing=False, fname=fname)
#         async_results.append(async_result)
...
#                 with open(fname, "rb") as f:
#                     re = pickle.load(f)
#                 print(str(n_results)+" out of "+str(len(async_results))+" aligned")
#                 at_ress.append(re)
...
def align_to_templates__batch_ipyparallel(op, data_json, segmentation_tg_op, tmp_dir, tem_keys):
    ...
    for f in os.listdir(tmp_dir):
        ...
        if not os.path.isfile(res_file):    continue
        with open(res_file, 'rb') as f:     at_ress_t = pickle.load(f)
        if type(at_ress_t)==list:
            at_ress.extend(at_ress_t)
        else:
            at_ress.append(at_ress_t)
    ...
    completed_subtomogram_set = set([_['vol_key']['subtomogram'] for _ in at_ress])
    align_count = len(completed_subtomogram_set)
    # assert align_count==len(at_ress)

    data_json_copy = [rec for rec in data_json if rec["subtomogram"] not in completed_subtomogram_set] # ignore those completed alignments
    print "Number of subtomograms that needs alignment calculation: " + str(len(data_json_copy))

    rc = ipyparallel.Client()
    view = rc.load_balanced_view()
    n_chunk = 10
    data_chunks = [data_json_copy[i:i+n_chunk] for i in range(0, len(data_json_copy), n_chunk)]
    print "Number of data_chunks created: " + str(len(data_chunks))
    ...
    for data_chunk in data_chunks:
        fname = os.path.join(tmp_dir, "tmp-"+str(uuid.uuid1())+".pickle")
        fnames.append(fname)
        async_results.append(view.apply_async(f=align_to_templates_ipyparallel, data_chunk=data_chunk, segmentation_tg_op=(segmentation_tg_op if op['template']['match']['use_segmentation_mask'] else None), tem_keys=tem_keys, align_op=op['align'], multiprocessing=False, fname=fname))

    print str(len(async_results)) + " tasks created"
    n_results = 0
    async_results_read = []
    while n_results < len(async_results):
        for i, ar in enumerate(async_results):
            if ar in async_results_read:
                continue
            if ar.ready():
                fname = fnames[i]
                ...
                print str(len(at_ress)) + " out of " + str(len(data_json))+ " aligned"
                n_results += 1
                del re
                #gc.collect()
                sys.stdout.write("\033[F")
                async_results_read.append(ar)
        time.sleep(1)
    del async_results, fnames, data_chunks, completed_subtomogram_set, data_json_copy, async_results_read
    gc.collect()    
    return at_ress

def align_to_templates_ipyparallel(data_chunk=None, segmentation_tg_op=None, tem_keys=None, template_wedge_cutoff=0.1, align_op=None, multiprocessing=False, fname=None):
    import tomominer.io.file as IV
    import ipyparallelUtil
    import numpy as N
    import pickle
    ...
    for rec in data_chunk:
        vi = None    
        if align_op['with_missing_wedge']:
            v =  IV.get_mrc(rec['subtomogram'])
            vm =  IV.get_mrc(rec['mask'])
        else:
            raise Exception('following options are need to be doube checked')
            if 'template' not in rec:  rec['template'] = None
            vi = ipyparallelUtil.impute_vol_keys_without_cache(vk=rec, ang=rec['angle'], loc=rec['loc'], tk=rec['template'], align_to_template=False, normalize=True)['vi']     # important: set align_to_template=False, so that subtomogram is not rotated, but template is rotated

        if (segmentation_tg_op is not None) and ('template' in rec) and ('segmentation' in rec['template']):
            v = ipyparallelUtil.align_to_templates__segment(rec=rec, v=v, segmentation_tg_op=segmentation_tg_op)['v']

        # record pairwise alignment results
        align_re = {}
        for c in tem_keys:
            align_re[c] = ipyparallelUtil.align_to_templates__pair_align(c=c, t_key=tem_keys[c], v=v, vm=vm, align_op=align_op)
            if N.isnan(align_re[c]['score']):
                print "alignment failed: rec %s, template %s, error %s ", repr(rec), repr(tem_keys[c]), repr(align_re[c]['err'])

        re.append({'vol_key':rec, 'align':align_re})
    
    with open(fname, "wb") as f:
        pickle.dump(re, f, protocol=0)
    return fname
